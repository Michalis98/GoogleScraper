...
func ScrapeClientRequest(searchURL string) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("Scraper received a non-200 code: %d", res.StatusCode)
		return nil, err
	}
...
func googleResultParsing(response *http.Response, rank int) ([]SearchResult, error) {
	...
	results := []SearchResult{}
	sel := doc.Find("div.g") // Verify this selector
	rank++
	...
	for i := range sel.Nodes {
		...
		titleTag := item.Find("h3") // Updated selector
